    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #key changes
        #1. forward_reward_weight: Increased from 1.25 to 5.0 to emphasize forward velocity
        #2. ctrl_cost_weight: Reduced from 0.1 to 0.05 to allow more dynamic actions
        #3. healthy_reward: Reduced from 7.0 to 2.0 to prevent it from dominating the reward
        #4. contact_cost_weight: Kept at 5e-7, but you should verify if cfrc_ext is non-zero (due to potential MuJoCo bugs)
    ):
    ...
    def _get_rew(self, x_velocity: float, action):
        ...
        foot_clearance_reward = 2.0 * max(right_foot_z, left_foot_z) if max(right_foot_z, left_foot_z) > 0.2 else 0.0
    ...
    def reset_model(self):
        ...
        qpos = self.init_qpos + self.np_random.uniform(
            low=noise_low, high=noise_high, size=self.model.nqhg
        )
